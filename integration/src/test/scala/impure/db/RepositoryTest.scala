...
class RepositoryTest extends BaseSpec2 {
      ...
      "return a list with all product rows" in {
        genProduct.sample match {
          ...
          case Some(p) =>
            ...
            } yield Product.fromDatabase(rows) match {
        ...
        genProduct.sample match {
          ...
          case Some(p) =>
            for {
              ...
              withClue("Data missing from database!")(cnts.sum must be(p.names.toNonEmptyList.size + 1))
        ...
        (genProduct.sample, genProduct.sample) match {
          case (Some(a), Some(b)) =>
            ...
            for {
              ...
              nosv <- repo.saveProduct(p).recover { case _ =>
                        0
                      }
        ...
        case (Some(a), Some(b)) =>
          ...
          for {
            ...
            withClue("Already existing product was not created!")(
              cnts.sum must be(a.names.toNonEmptyList.size + 1)
            )
            Product.fromDatabase(rows) match {
              case None    => fail("No product created from database rows!")
              case Some(c) =>
  ...
  "The product does not exist" must {
    "return an error not change the database" in {
      genProduct.sample match {
        ...
        case Some(p) =>
          for {
            nosv <- repo.updateProduct(p).recover { case _ =>
                      0
                    }
            rows <- repo.loadProduct(p.id)
          } yield {
